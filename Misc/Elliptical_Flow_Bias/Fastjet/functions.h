...
//----------------------------------------------------------------------------------------
TH1F* Modify_hist_old(TH1F* hist_input, Double_t mean_shift, Double_t width_fac, Double_t width_shift, Long64_t sample_val)
{
    ...
    TRandom3 r3b_hist;
    r3b_hist.SetSeed(1); // seed for random number generator changes every second
    ...
    for(Int_t ibin = 1; ibin <= hist_input->GetNbinsX(); ibin++)
    {
        ...
        //hist_output->SetBinError(ibin, TMath::Sqrt(hist_output->GetBinContent(ibin)));
    }
...
TH1F* Modify_hist(TH1F* hist_input, Double_t mean_shift, Double_t width_fac, Double_t width_shift, Long64_t sample_val)
{
    ...
    // Get Maximum bin
    Int_t    max_bin      = hist_input ->GetMaximumBin();
    Double_t bin_cont_min = hist_input ->GetBinContent(max_bin);

    Double_t scale_fac = 0.0;
    if(bin_cont_min > 0) scale_fac = TMath::Power(hist_input->GetBinError(max_bin),2)/(bin_cont_min);
    //cout << "Scale fac functions.h " << scale_fac << endl;
    hist_input->Scale(1/scale_fac);
    ...
    for(Int_t ibin = 1; ibin <= hist_input->GetNbinsX(); ibin++)
    {
        ...
        hist_output->SetBinError(ibin, TMath::Sqrt(hist_output->GetBinContent(ibin)));     // New, added 18 Jun 24

    }

    if(bin_cont_min > 0.0) hist_output->Scale(bin_cont_min);

    // Scale again to original input histo
    hist_output->Scale(scale_fac);
    hist_input->Scale(scale_fac);

    return hist_output;
}
//----------------------------------------------------------------------------------------
TVector3 calc_scaling_factor(TH1D* h_SE, TH1D* h_ME, Double_t start_int, Double_t stop_int, Int_t N_steps)
{
    Double_t best_chi2 = 100000000.0;
    Double_t best_width_scale = 0.0;
    Long64_t sampling = 1000000;
    TVector3 vec_scale_start_stop_int;
    for(Int_t i_mod = 0; i_mod < N_steps; i_mod++)
    {
        Double_t width_scale = 1.0 + 0.001*i_mod;
        TH1F* h_ME_mod = Modify_hist((TH1F*)h_ME,0.0,width_scale,0.0,sampling); // TH1F* hist_input, Double_t mean_shift, Double_t width_fac, Double_t width_shift, Long64_t sample_val
        for(Int_t i_bin = 1; i_bin <= h_ME_mod->GetNbinsX(); i_bin++)
        {
            Double_t bin_error = h_ME ->GetBinError(i_bin);
            h_ME_mod ->SetBinError(i_bin,bin_error);
        }

        Double_t integral_SE     = h_SE     ->Integral(h_SE->GetXaxis()->FindBin(start_int),h_SE->GetXaxis()->FindBin(stop_int));
        Double_t integral_ME     = h_ME     ->Integral(h_ME->GetXaxis()->FindBin(start_int),h_ME->GetXaxis()->FindBin(stop_int));
        Double_t integral_ME_mod = h_ME_mod ->Integral(h_ME_mod->GetXaxis()->FindBin(start_int),h_ME_mod->GetXaxis()->FindBin(stop_int));

        h_ME     ->Scale(integral_SE/integral_ME);
        h_ME_mod ->Scale(integral_SE/integral_ME_mod);

        TH1D* h_SE_ME_ratio     = (TH1D*)h_SE->Clone("h_SE_ME_ratio");
        TH1D* h_SE_ME_mod_ratio = (TH1D*)h_SE->Clone("h_SE_ME_ratio");

        h_SE_ME_ratio     ->Divide(h_ME);
        h_SE_ME_mod_ratio ->Divide(h_ME_mod);

        Double_t sum_chi2 = 0.0;
        Int_t n_points_used = 0;
        //for(Int_t i_bin = h_SE_ME_mod_ratio->GetXaxis()->FindBin(-27.0); i_bin < h_SE_ME_mod_ratio->GetXaxis()->FindBin(stop_int); i_bin++)
        //for(Int_t i_bin = 1; i_bin < h_SE_ME_mod_ratio->GetXaxis()->FindBin(stop_int); i_bin++)
        for(Int_t i_bin = 1; i_bin < h_SE_ME_mod_ratio->GetXaxis()->FindBin(-15); i_bin++)
        {
            Double_t bin_cont  = h_SE_ME_mod_ratio ->GetBinContent(i_bin);
            Double_t bin_cent  = h_SE_ME_mod_ratio ->GetBinCenter(i_bin);
            Double_t bin_error = h_SE_ME_mod_ratio ->GetBinError(i_bin);
            //bin_error = 0.01;
            if(bin_cont > 0.0 && bin_error > 0.0)
            {
                Double_t chi2 = TMath::Power(bin_cont - 1.0,2)/TMath::Power(bin_error,2);
                sum_chi2 += chi2;
                n_points_used++;
                //printf("n_points_used: %d, bin_cent: %4.3f, bin_cont: %4.3f, chi2: %4.3f \n",n_points_used,bin_cent,bin_cont,chi2);
            }
            //printf("bin_cont: %4.3f, bin_error: %4.3f \n",bin_cont,bin_error);
        }


        if(sum_chi2 < best_chi2 && n_points_used > 0)
        {
            best_chi2 = sum_chi2;
            best_width_scale = width_scale;
            vec_scale_start_stop_int.SetXYZ(best_width_scale,best_chi2,0.0);
        }
        Double_t width_percent = 100*(width_scale - 1.0);
        //printf("width_scale: %4.4f, sum_chi2: %4.4f \n",width_scale,sum_chi2);

        h_ME_mod          ->Delete();
        h_SE_ME_ratio     ->Delete();
        h_SE_ME_mod_ratio ->Delete();
    }

    //return best_width_scale;
    return vec_scale_start_stop_int;
...
//----------------------------------------------------------------------------------------
Double_t LinFitFunc(Double_t* x_val, Double_t* par)
{
    Double_t x, y, m, b;
    m  = par[0];
    b  = par[1];
...
//------------------------------------------------------------------------------------
Double_t get_pT_region_for_chi2(Int_t i_radius, TH1D* h_SE_ME_ratio_fit)
{
    vector<vector<Double_t>> vec_stop_value_var; 
    vec_stop_value_var.clear();
    vec_stop_value_var.resize(4);
    vec_stop_value_var[0].push_back(-15); 
    vec_stop_value_var[0].push_back(-14); 
    vec_stop_value_var[0].push_back(-13); 
    vec_stop_value_var[0].push_back(-12); 
    vec_stop_value_var[1].push_back(-25); 
    vec_stop_value_var[1].push_back(-20); 
    vec_stop_value_var[1].push_back(-15); 
    vec_stop_value_var[1].push_back(-12); 
    vec_stop_value_var[2].push_back(-30); 
    vec_stop_value_var[2].push_back(-25); 
    vec_stop_value_var[2].push_back(-20); 
    vec_stop_value_var[2].push_back(-18); 
    vec_stop_value_var[3].push_back(-40); 
    vec_stop_value_var[3].push_back(-35); 
    vec_stop_value_var[3].push_back(-30); 
    vec_stop_value_var[3].push_back(-25); 

    Double_t start_value[4] = {-18, -30, -40, -55}; 

    TF1* fit_func = new TF1("LinFitFunc",LinFitFunc,-100,100,2);
    fit_func->SetParameter(0,0.5);
    fit_func->SetParameter(1,1.5);

    TGraph* tg_SE_ME_ratio_fit = new TGraph(); 

    Int_t start_bin = h_SE_ME_ratio_fit->GetXaxis()->FindBin(start_value[i_radius]); 
    Int_t stop_bin = h_SE_ME_ratio_fit->GetXaxis()->FindBin(vec_stop_value_var[i_radius][0]); 
    
    for(Int_t i_bin = start_bin; i_bin < stop_bin; i_bin++)
    {
        Double_t bin_cont = h_SE_ME_ratio_fit->GetBinContent(i_bin);
        Double_t bin_center = h_SE_ME_ratio_fit->GetBinCenter(i_bin);
        tg_SE_ME_ratio_fit->SetPoint(i_bin, bin_center, bin_cont); 
    }
    //tg_SE_ME_ratio_fit->Fit(fit_func, "MN","MN",start_value,vec_stop_value_var[i_radius][0]);

    return vec_stop_value_var[i_radius][0];
...
Double_t calc_norm_upper_pT(TH1D* h_SE, TH1D* h_ME, Int_t i_radius, Double_t start_int_value, Double_t stop_int_value_iter)
{   
    
    vector<Int_t> vec_stop_int_values_test; 
    vec_stop_int_values_test.clear();
    Int_t start_int_bin = h_SE ->FindBin(start_int_value);
    Int_t stop_int_bin_full = h_SE->FindBin(stop_int_value_iter);
    for(Int_t i_bin = start_int_bin; i_bin < stop_int_bin_full; i_bin++)
    {
        vec_stop_int_values_test.push_back(i_bin);
    }
    Double_t SE_norm_value, ME_norm_value;
    Double_t SE_norm_value_err, ME_norm_value_err;
    Double_t scaling_factor = 1.0;
    Double_t scaling_factor_plus = 1.0;
    Double_t scaling_factor_minus = 1.0;
    Double_t scaling_factor_err = 1.0;
    Double_t integral_SE_ME_diff = 0.0; 
    Double_t integral_SE_ME_diff_plus = 0.0; 
    Double_t integral_SE_ME_diff_minus = 0.0; 
    Double_t ratio_int_SE_ME_diff = 1.0; 
    Double_t upper_limit = -9999.9; 

    TH1D* h_ME_scaling;  
    TH1D* h_ME_scaling_plus;  
    TH1D* h_ME_scaling_minus;  
    TH1D* h_SE_ME_diff; 
    TH1D* h_SE_ME_diff_plus; 
    TH1D* h_SE_ME_diff_minus; 

    for(Int_t i_iteration = 0; i_iteration < (Int_t)vec_stop_int_values_test.size(); i_iteration++)
    {
        SE_norm_value = h_SE->Integral(start_int_bin,vec_stop_int_values_test[i_iteration]);
        ME_norm_value = h_ME->Integral(start_int_bin,vec_stop_int_values_test[i_iteration]);
        
        SE_norm_value_err = TMath::Sqrt(SE_norm_value); 
        ME_norm_value_err = TMath::Sqrt(ME_norm_value);   

        //cout << "upper limit: "<< h_SE->GetBinCenter(vec_stop_int_values_test[i_iteration]) << ", SE_norm_value: " << SE_norm_value << ", +/- "<< SE_norm_value_err << ", ME_norm_value: " << ME_norm_value << ", +/- "<< ME_norm_value_err << endl;
        if(ME_norm_value <= 0.0 || SE_norm_value <= 0.0) continue; 
        scaling_factor = SE_norm_value/ME_norm_value;
        
        
        scaling_factor_err = TMath::Sqrt(TMath::Power((SE_norm_value_err/ME_norm_value),2)+TMath::Power(((ME_norm_value_err*SE_norm_value)/(ME_norm_value*ME_norm_value)),2));
        //cout << "scaling_factor: " << scaling_factor << " scaling_factor_err: "<< scaling_factor_err << endl;

        Double_t rel_error_scaling_factor = fabs(scaling_factor_err/scaling_factor); 
        //cout << "upper limit: "<< h_SE->GetBinCenter(vec_stop_int_values_test[i_iteration]) << " ,scaling_factor: " << scaling_factor << ", rel_err_scaling_factor_plus: " << rel_error_scaling_factor << endl;
#if 1
        if(fabs(rel_error_scaling_factor) < 0.1) 
        {
            //cout<< "upper limit: "<< h_SE->GetBinCenter(vec_stop_int_values_test[i_iteration]) << ", ratio: " << ratio_int_SE_ME_diff <<", delta_integral_SE_ME_diff: " << delta_integral_SE_ME_diff << endl;
            //cout << "upper limit: "<< h_SE->GetBinCenter(vec_stop_int_values_test[i_iteration]) << " ,scaling_factor: " << scaling_factor << ", rel_err_scaling_factor_plus: " << rel_error_scaling_factor << endl;
     
            upper_limit = h_SE->GetBinCenter(vec_stop_int_values_test[i_iteration]);
            break; 
        }
#endif
#if 0
        scaling_factor_plus  = scaling_factor + scaling_factor_err; 
        scaling_factor_minus = scaling_factor - scaling_factor_err;  
        //cout << "upper limit: "<< h_SE->GetBinCenter(vec_stop_int_values_test[i_iteration]) << " ,scaling_factor: " << scaling_factor << ", scaling_factor_plus: " << scaling_factor_plus << ", scaling_factor_minus: " << scaling_factor_minus  << endl;
        
        TH1D* h_ME_scaling = (TH1D*)h_ME->Clone("h_ME_scaling");
        TH1D* h_ME_scaling_plus = (TH1D*)h_ME->Clone("h_ME_scaling_plus");
        TH1D* h_ME_scaling_minus = (TH1D*)h_ME->Clone("h_ME_scaling_minus");
        
        h_ME_scaling       ->Scale(scaling_factor);
        h_ME_scaling_plus  ->Scale(scaling_factor_plus);
        h_ME_scaling_minus ->Scale(scaling_factor_minus);

        TH1D* h_SE_ME_diff       = (TH1D*)h_SE->Clone(HistName.Data());
        TH1D* h_SE_ME_diff_plus  = (TH1D*)h_SE->Clone(HistName.Data());
        TH1D* h_SE_ME_diff_minus = (TH1D*)h_SE->Clone(HistName.Data());
        h_SE_ME_diff       ->Add(h_ME_scaling,-1.0); 
        h_SE_ME_diff_plus  ->Add(h_ME_scaling_plus,-1.0); 
        h_SE_ME_diff_minus ->Add(h_ME_scaling_minus,-1.0); 


        integral_SE_ME_diff       = h_SE_ME_diff->Integral();
        integral_SE_ME_diff_plus  = h_SE_ME_diff_plus->Integral();
        integral_SE_ME_diff_minus = h_SE_ME_diff_minus->Integral();
        //cout <<"upper limit: "<< h_SE->GetBinCenter(vec_stop_int_values_test[i_iteration]) <<", integral_SE_ME_diff: " << integral_SE_ME_diff << ", integral_SE_ME_diff_plus: "<< integral_SE_ME_diff_plus<< ", integral_SE_ME_diff_minus: " << integral_SE_ME_diff_minus << endl;
        //cout << "integral_SE_ME_diff: " << integral_SE_ME_diff << ", integral_SE_ME_diff_plus: "<< integral_SE_ME_diff-integral_SE_ME_diff_plus<< ", integral_SE_ME_diff_minus: " << integral_SE_ME_diff_minus-integral_SE_ME_diff << endl;
        if(integral_SE_ME_diff == 0.0) continue; 
        Double_t delta_integral_SE_ME_diff = fabs(integral_SE_ME_diff_plus-integral_SE_ME_diff_minus);
        ratio_int_SE_ME_diff = 0.5*delta_integral_SE_ME_diff/integral_SE_ME_diff; 

        //cout<< "upper limit: "<< h_SE->GetBinCenter(vec_stop_int_values_test[i_iteration]) << ", ratio: " << ratio_int_SE_ME_diff <<", delta_integral_SE_ME_diff: " << delta_integral_SE_ME_diff << endl;

        if(fabs(ratio_int_SE_ME_diff) < 0.5) 
        {
            cout<< "upper limit: "<< h_SE->GetBinCenter(vec_stop_int_values_test[i_iteration]) << ", ratio: " << ratio_int_SE_ME_diff <<", delta_integral_SE_ME_diff: " << delta_integral_SE_ME_diff << endl;
            upper_limit = h_SE->GetBinCenter(vec_stop_int_values_test[i_iteration]);
            break; 
        ...
        h_ME_scaling->Delete();
        h_ME_scaling_plus->Delete();
        h_ME_scaling_minus->Delete();

        h_SE_ME_diff  ->Delete();    
        h_SE_ME_diff_plus  ->Delete();
        h_SE_ME_diff_minus ->Delete();
#endif
    }
    return upper_limit; 
...
Double_t calc_norm_upper_pT_global(TH1D* h_SE, TH1D* h_ME, Int_t i_radius, Double_t start_int_value)
{   
    
    vector<Int_t> vec_stop_int_values_test; 
    vec_stop_int_values_test.clear();
    Int_t start_int_bin = h_SE ->FindBin(start_int_value);
    Int_t stop_int_bin_full = h_SE->FindBin(0.0);
    for(Int_t i_bin = start_int_bin; i_bin < stop_int_bin_full; i_bin++)
    {
        vec_stop_int_values_test.push_back(i_bin);
    }
...
void calc_syst_var_scaling_factor(Int_t i_radius, TH1D* h_SE, TH1D* h_ME, Double_t start_int, vector<vector<Double_t>> stop_int, Int_t N_steps, Double_t step_size, vector<vector<Double_t>> &out, Double_t width_shift)
{
    Double_t best_chi2 = 100000000.0;
    Double_t best_width_scale = 0.0;
    Long64_t sampling = 1000000;
    Double_t chi2_range[4] = {-10,-15,-20,-15};
    out.resize(4);
    vector<vector<Double_t>> chi2_scale_int;
    chi2_scale_int.clear();
    chi2_scale_int.resize(4);
    
    for(Int_t i_norm = 0; i_norm < (Int_t)stop_int[i_radius].size(); i_norm++)
    {
        for(Int_t i_mod = 0; i_mod < N_steps; i_mod++)
        {
            Double_t integral_SE     = h_SE     ->Integral(h_SE->GetXaxis()->FindBin(start_int),h_SE->GetXaxis()->FindBin(stop_int[i_radius][i_norm]));
            Double_t integral_ME     = h_ME     ->Integral(h_ME->GetXaxis()->FindBin(start_int),h_ME->GetXaxis()->FindBin(stop_int[i_radius][i_norm]));

            Double_t width_scale = 1.0 + step_size*i_mod;
            //cout << "width_scale: "<< width_scale << endl;
            TH1F* h_ME_mod = Modify_hist((TH1F*)h_ME,0.0,width_scale,width_shift,sampling); // TH1F* hist_input, Double_t mean_shift, Double_t width_fac, Double_t width_shift, Long64_t sample_val
            //TH1F* h_ME_mod = Modify_hist((TH1F*)h_ME,0.0,width_scale,15.0,sampling);
            for(Int_t i_bin = 1; i_bin <= h_ME_mod->GetNbinsX(); i_bin++)
            {
                Double_t bin_error = h_ME ->GetBinError(i_bin);
                //h_ME_mod ->SetBinError(i_bin,bin_error);
            ...
            Double_t integral_ME_mod = h_ME_mod ->Integral(h_ME_mod->GetXaxis()->FindBin(start_int),h_ME_mod->GetXaxis()->FindBin(stop_int[i_radius][i_norm]));
            h_ME_mod ->Scale(integral_SE/integral_ME_mod);
            ...
            //chi2_range[i_radius] = get_pT_region_for_chi2(i_radius,h_SE_ME_mod_ratio);
            //cout << "chi2 range: " << chi2_range[i_radius] << endl;

            Double_t sum_chi2 = 0.0;
            Double_t reduced_chi2 = 0.0;
            Int_t n_points_used = 0;
            //for(Int_t i_bin = 1; i_bin < h_SE_ME_mod_ratio->GetXaxis()->FindBin(chi2_range[i_radius]); i_bin++)
            //
            for(Int_t i_bin = 1; i_bin < h_SE_ME_mod_ratio->GetXaxis()->FindBin(stop_int[i_radius][i_norm]); i_bin++)
            //for(Int_t i_bin = 1; i_bin < h_SE_ME_mod_ratio->GetXaxis()->FindBin(-35); i_bin++)
            {
                Double_t bin_cont  = h_SE_ME_mod_ratio ->GetBinContent(i_bin);
                Double_t bin_cent  = h_SE_ME_mod_ratio ->GetBinCenter(i_bin);
                Double_t bin_error = h_SE_ME_mod_ratio ->GetBinError(i_bin);
            ...
            if(n_points_used > 0) reduced_chi2 =  sum_chi2 /(Double_t)n_points_used;
/*
            if(sum_chi2 < best_chi2 && n_points_used > 0)
            {
                best_chi2 = sum_chi2;
            ...
            */
            if(reduced_chi2 < best_chi2 && n_points_used > 0)
            {
                best_chi2 = reduced_chi2;
                best_width_scale = width_scale;
            }

            chi2_scale_int[0].push_back(width_scale);
            chi2_scale_int[1].push_back(start_int);
            chi2_scale_int[2].push_back(stop_int[i_radius][i_norm]);
            chi2_scale_int[3].push_back(sum_chi2);

            Double_t width_percent = 100*(width_scale - 1.0);
            //printf("norm: %4.4f, width_scale: %4.4f, sum_chi2: %4.4f \n",stop_int[i_radius][i_norm],width_scale,sum_chi2);
    ...
    best_chi2 = 100000000.0;
    for(Int_t i_entry = 0; i_entry < (Int_t)chi2_scale_int[3].size(); i_entry++)
    {
        if(chi2_scale_int[3][i_entry] < best_chi2) best_chi2 = chi2_scale_int[3][i_entry];
    }
    //cout << "best chi2: " << best_chi2 << endl;

    for(Int_t i_entry = 0; i_entry < (Int_t)chi2_scale_int[3].size(); i_entry++)
    {
        if(chi2_scale_int[3][i_entry] < (best_chi2+1))
        {
            out[0].push_back(chi2_scale_int[0][i_entry]);
            out[1].push_back(chi2_scale_int[1][i_entry]);
            out[2].push_back(chi2_scale_int[2][i_entry]);
            out[3].push_back(chi2_scale_int[3][i_entry]);
        }
...
void calc_syst_var_scaling_factor_penalty(Int_t i_radius, TH1D* h_SE, TH1D* h_ME, Double_t start_int, vector<vector<Double_t>> stop_int, Int_t N_steps, Double_t step_size, vector<vector<Double_t>> &out, Double_t width_shift)
{
    Double_t best_chi2 = 100000000.0;
    Double_t best_width_scale = 0.0;
    Long64_t sampling = 1000000;
    ...
    for(Int_t i_norm = 0; i_norm < (Int_t)stop_int[i_radius].size(); i_norm++)
    {
        for(Int_t i_mod = 0; i_mod < N_steps; i_mod++)
        {
            ...
            Double_t sum_chi2_penalty = 0.0;
            Int_t n_points_used_penalty = 0;
            for(Int_t i_bin =  h_SE_ME_mod_ratio->GetXaxis()->FindBin(stop_int[i_radius][i_norm]); i_bin < h_SE_ME_mod_ratio->GetXaxis()->FindBin(stop_int[i_radius][i_norm]+50.0); i_bin++)
            {
                Double_t bin_cont  = h_SE_ME_mod_ratio ->GetBinContent(i_bin);
                Double_t bin_cent  = h_SE_ME_mod_ratio ->GetBinCenter(i_bin);
                Double_t bin_error = h_SE_ME_mod_ratio ->GetBinError(i_bin);
                if(bin_cont > 0.0 && bin_error > 0.0)
                {
                    Double_t diff_ratio_one = bin_cont - 1.0; 
                    Double_t chi2_penalty = 0.0; 
                    if(diff_ratio_one < 0.0) chi2_penalty = TMath::Power(bin_cont - 1.0,2)/TMath::Power(bin_error,2);
                    
                    //cout << "bin error: " << bin_error << endl;
                    sum_chi2_penalty += chi2_penalty;
                    n_points_used_penalty++;
                    printf("n_points_used_penalty: %d, bin_cent: %4.3f, bin_cont: %4.3f, chi2: %4.3f \n",n_points_used_penalty,bin_cent,bin_cont,chi2_penalty);
                }
                //printf("bin_cont: %4.3f, bin_error: %4.3f \n",bin_cont,bin_error);
            }
#endif
            cout << "sum_chi2 penalty: " << sum_chi2_penalty << endl;
            
            sum_chi2 += sum_chi2_penalty; 
            sum_chi2 *= 0.5; 
...
//----------------------------------------------------------------------------------------
void calc_scaling_factor_categories(Int_t i_radius, TH1D* h_SE, TH1D* h_ME, Double_t start_int, Double_t stop_int, Int_t N_steps, Double_t step_size, vector<vector<Double_t>> &out)
{
    Double_t best_chi2 = 100000000.0;
    Double_t best_width_scale = 0.0;
    Long64_t sampling = 1000000;
...
/*
        ...
        */
        ...
        chi2_scale_int[0].push_back(width_scale);
        chi2_scale_int[1].push_back(start_int);
        chi2_scale_int[2].push_back(stop_int);
        chi2_scale_int[3].push_back(sum_chi2);
...
//----------------------------------------------------------------------------------------
#if 0
TH1D* rebin_histo(TH1D* h_input, Double_t* bins, Int_t n_bins)
{
    cout << "start rebin" << endl;
    vector<Int_t> N_bins;
    N_bins.clear();
    N_bins.resize(n_bins);
    vector<Double_t> rebin_error;
    rebin_error.clear();
    rebin_error.resize(n_bins);

    for(Int_t i_bin = 0; i_bin < n_bins; i_bin++)
    {
        N_bins[i_bin] = 0;
        rebin_error[i_bin] = 0;
    }

    TH1D* h_rebin = new TH1D();
    h_rebin->SetBins(n_bins-1,bins);

    for(Int_t i_bin = 1; i_bin <= h_input->GetNbinsX(); i_bin++)
    {
        Double_t bin_content = h_input->GetBinContent(i_bin);
        Double_t bin_center  = h_input->GetBinCenter(i_bin);
        Double_t bin_err     = h_input->GetBinError(i_bin);
        //cout <<"i_bin: "<< i_bin  << ", bin_center: " << bin_center << ", bin_content: "<< bin_content <<", bin error: "<< bin_err <<endl;

        Int_t i_rebin = h_rebin->GetXaxis()->FindBin(bin_center);
        Double_t bin_center_rebin = h_rebin->GetBinCenter(i_rebin);
        //cout << "i_rebin: "  << i_rebin <<", bin_center_rebin: "<< bin_center_rebin<< endl;
        N_bins[i_rebin]++;
        rebin_error[i_rebin] += bin_err*bin_err;
        //h_input_syst_var_rebin[i_histo]->SetBinContent(i_rebin,bin_content);
        h_rebin->Fill(bin_center,bin_content);
    }
    //cout << "N bins: " << h_rebin->GetNbinsX() << endl;
    for(Int_t i_bin = 1; i_bin < h_rebin->GetNbinsX()+1; i_bin++)
    {
        //cout << "i_bin: " << i_bin << ", N_bins_meregd: "<< N_bins[i_bin]<< ", bin_error: "<<  TMath::Sqrt(rebin_error[i_bin]) << endl;

        Double_t bin_content =  h_rebin ->GetBinContent(i_bin);
        Double_t bin_error   =  h_rebin ->GetBinError(i_bin);

        h_rebin ->SetBinContent(i_bin, bin_content/N_bins[i_bin]);
        h_rebin ->SetBinError(i_bin, TMath::Sqrt(rebin_error[i_bin])/N_bins[i_bin]);
...
TH1D* rebin_histo(TH1D* h_input, Double_t* bins, Int_t n_bins)
{
    ...
    rebin_error.clear();
    rebin_error.resize(n_bins);
    vector<Double_t> bin_content_sum;
    bin_content_sum.clear();
    bin_content_sum.resize(n_bins);

    for(Int_t i_bin = 0; i_bin < n_bins; i_bin++)
    {
        N_bins[i_bin] = 0;
        rebin_error[i_bin] = 0;
        bin_content_sum[i_bin] = 0;
    }
    ...
    for(Int_t i_bin = 1; i_bin <= h_input->GetNbinsX(); i_bin++)
    {
        ...
        N_bins[i_rebin]++;
        rebin_error[i_rebin] += bin_err*bin_err;
        bin_content_sum[i_rebin]+= bin_content;
    ...
    for(Int_t i_bin = 1; i_bin < h_rebin->GetNbinsX()+1; i_bin++)
    {
        ...
        Double_t bin_content_norm = bin_content_sum[i_bin];
        if(N_bins[i_bin] != 0) bin_content_norm = bin_content_sum[i_bin]/N_bins[i_bin];
        Double_t bin_err_norm = TMath::Sqrt(rebin_error[i_bin]);
        if(N_bins[i_bin] != 0) bin_err_norm = TMath::Sqrt(rebin_error[i_bin])/N_bins[i_bin];

        h_rebin ->SetBinContent(i_bin, bin_content_norm);
        h_rebin ->SetBinError(i_bin, bin_err_norm);
...
//----------------------------------------------------------------------------------------
TH2D* rebin_histo_2D(TH2D* h2D_input, Double_t* binsX,Double_t* binsY, Int_t n_binsX, Int_t n_binsY)
{
    cout << "start rebin" << endl;
    vector<vector<Int_t>> N_bins;
    N_bins.clear();
    N_bins.resize(n_binsX);
    vector<vector<Double_t>> rebin_error;
    rebin_error.clear();
    rebin_error.resize(n_binsX);

    for(Int_t i_binX = 0; i_binX < n_binsX; i_binX++)
    {
        N_bins[i_binX].resize(n_binsY);
        rebin_error[i_binX].resize(n_binsY);
    }

    for(Int_t i_binX = 0; i_binX < n_binsX; i_binX++)
    {
        for(Int_t i_binY = 0; i_binY < n_binsY; i_binY++)
        {
            N_bins[i_binX][i_binY] = 0;
            rebin_error[i_binX][i_binY] = 0;
        }
    }

    TH2D* h2D_rebin = new TH2D();
    h2D_rebin->SetBins(n_binsX-1,binsX,n_binsY-1,binsY);

    for(Int_t i_binX = 1; i_binX <= h2D_input->GetNbinsX(); i_binX++)
    {
        for(Int_t i_binY = 1; i_binY <= h2D_input->GetNbinsY(); i_binY++)
        {

            Double_t bin_content  = h2D_input->GetBinContent(i_binX,i_binY);
            Double_t bin_centerX  = h2D_input->GetXaxis()->GetBinCenter(i_binX);
            Double_t bin_centerY  = h2D_input->GetYaxis()->GetBinCenter(i_binY);
            Double_t bin_err      = h2D_input->GetBinError(i_binX,i_binY);
            //cout <<"i_bin: "<< i_bin  << ", bin_center: " << bin_center << ", bin_content: "<< bin_content <<", bin error: "<< bin_err <<endl;

            Int_t i_rebinX = h2D_rebin->GetXaxis()->FindBin(bin_centerX);
            Int_t i_rebinY = h2D_rebin->GetYaxis()->FindBin(bin_centerY);
            Double_t bin_center_rebinX = h2D_rebin->GetXaxis()->GetBinCenter(i_rebinX);
            Double_t bin_center_rebinY = h2D_rebin->GetYaxis()->GetBinCenter(i_rebinY);
            //cout << "i_rebin: "  << i_rebin <<", bin_center_rebin: "<< bin_center_rebin<< endl;
            N_bins[i_rebinX][i_rebinY]++;
            rebin_error[i_rebinX][i_rebinY] += bin_err*bin_err;
            h2D_rebin->Fill(bin_centerX,bin_centerY,bin_content);
        }
    }
    //cout << "N bins: " << h_rebin->GetNbinsX() << endl;
    for(Int_t i_binY = 1; i_binY < h2D_rebin->GetNbinsY()+1; i_binY++)
    {
        for(Int_t i_binX = 1; i_binX <  h2D_rebin->GetNbinsX()+1; i_binX++)
        {
            //cout << "i_bin: " << i_bin << ", N_bins_meregd: "<< N_bins[i_bin]<< ", bin_error: "<<  TMath::Sqrt(rebin_error[i_bin]) << endl;
            Double_t bin_cont   =  h2D_rebin->GetBinContent(i_binX,i_binY);
            Double_t bin_error   =  h2D_rebin ->GetBinError(i_binX,i_binY);

            h2D_rebin ->SetBinContent(i_binX,i_binY, bin_cont/N_bins[i_binX][i_binY]);
            h2D_rebin ->SetBinError(i_binX,i_binY, TMath::Sqrt(rebin_error[i_binX][i_binY])/N_bins[i_binX][i_binY]);
        }
    }
    return h2D_rebin;
}
...
//----------------------------------------------------------------------------------------
TGraphAsymmErrors*  rebin_histo_tgae(TH1D* th1d_in, vector<Double_t> bins)
{
    TGraphAsymmErrors* tgae_rebinned = new TGraphAsymmErrors();
    Double_t x_val = 0;
    Double_t y_val = 0 ;
    Double_t err_X_high ;
    Double_t err_X_low ;
    Double_t bin_width_new;
    Int_t start_int_bin, stop_int_bin;

    Double_t integral = 0.0;

    for(Int_t i_point = 0; i_point < (Int_t)bins.size()-1; i_point+=1)
    {
        //cout<< "i_point: "<<i_point<<endl;

        Double_t bin_low_edge = bins[i_point];
        Double_t bin_low_edge_next = bins[i_point+1];
        x_val = 0.5 * (bin_low_edge_next+bin_low_edge);
        err_X_high = bin_low_edge_next - x_val;
        err_X_low = x_val - bin_low_edge;


        //cout <<"x_val: "<< x_val <<"bin_low_edge: "<< bin_low_edge<<endl;
        start_int_bin = th1d_in ->FindBin(bin_low_edge);
        stop_int_bin  = th1d_in->FindBin(bin_low_edge_next);
        for(Int_t i_bin = start_int_bin; i_bin < stop_int_bin; i_bin++)
        {
            y_val += th1d_in->GetBinContent(i_bin);
        }

        cout <<"x_val: "<< x_val <<", y_val: "<< y_val <<endl;
        tgae_rebinned ->SetPoint(i_point,x_val,y_val/(err_X_high + err_X_low) );
        tgae_rebinned ->SetPointEXhigh(i_point,err_X_high);
        tgae_rebinned ->SetPointEXlow(i_point,err_X_low);

        x_val = 0;
        y_val = 0;

    }
    return tgae_rebinned;
}
//------------------------------------------------------------------------------------------------------------
...
TCanvas* Draw_1D_graph_and_canvas(TGraph* hist, TString name, Int_t x_size, Int_t y_size,
                                  ...
                                  Int_t style, Double_t size, Int_t color,Double_t min_val_x, Double_t max_val_x)
{
    ...
    if(min_val_x != max_val_x) hist->GetXaxis()->SetRangeUser(min_val_x,max_val_x);
...
//----------------------------------------------------------------------------------------
Double_t LogFunc(Double_t* z_val, Double_t* par)
{
    Double_t z_vertex, r, EPD_z_pos, flag_dir;
    r         = par[0];
    EPD_z_pos = par[1];
...
//----------------------------------------------------------------------------------------
Double_t PolyFitFunc_c(Double_t* x_val, Double_t* par)
{
    ...
    par6  = par[6];
    x = x_val[0];
    y = par0 + par1*x + par2*x*x + par3*x*x*x + par4*x*x*x*x + par5*x*x*x*x*x + par6*x*x*x*x*x*x;
    return y;
}

//----------------------------------------------------------------------------------------
Double_t SigmFunc(Double_t* x_val, Double_t* par)
{
    ...
    x = x_val[0];
    y = (par2/(1+ TMath::Exp(-par0*(x-par1))));

    return y;
}
//----------------------------------------------------------------------------------------
Double_t SigmFunctest(Double_t* x_val, Double_t* par)
{
    Double_t x, y, par0, par1, par2,par3;
    par0  = par[0];
    par1  = par[1];
    ...
    x = x_val[0];
    y = (par3/(1+ TMath::Exp(-par0*(x-par1))));
    //y -= par2;
    return y;
}
//----------------------------------------------------------------------------------------
Double_t TestErf(Double_t* x_val, Double_t* par)
{
    Double_t x, y, par0, par1,par2;
    par0  = par[0];
    par1  = par[1];
    x = x_val[0];
    //y = par0*(TMath::Erf((x-par1)/100));
    y = par0*(TMath::Erf((x)/par1));
...
//----------------------------------------------------------------------------------------
Double_t PolyFitFunc_b(Double_t* x_val, Double_t* par)
{
    Double_t x, y, par0, par1, par2, par3, par4, par5;
    par0  = par[0];
    par1  = par[1];
...
void SetRootGraphicStyle()
{
    ...
    gStyle->SetTextFont(20);
    gStyle->SetTitleFont(42, "xyz");
    gStyle->SetLabelFont(20, "xyz");
...
void Monte_Carlo_Error_Ratio::calc_errors()
{
    ...
    if(RMS[0] >= 0.0)
    {
        for(Int_t i_MC = 0; i_MC < N_MC; i_MC++)
        {
            ...
            Double_t val_nom = (ran.Rndm()-0.5)*2.0*nom_err+nom;
            Double_t val_den = (ran.Rndm()-0.5)*2.0*den_err+den;
...
//----------------------------------------------------------------------------------------
TCanvas* Draw_1D_histo_and_canvas_V2(TH1D* hist, TString name, TString x_title, TString y_title, Int_t x_size, Int_t y_size,
                                  Double_t min_x_val, Double_t max_x_val, Double_t min_y_val, Double_t max_y_val,
                                  Double_t title_offset_X, Double_t title_offset_Y, Double_t label_size,
                                  Int_t line_width, Int_t line_color, Int_t fill_style, Int_t fill_color, TString option, Int_t log)
{
    // 05.07.2021
    TCanvas* canvas = new TCanvas(name.Data(),name.Data(),10,10,x_size,y_size);
    canvas->SetFillColor(10);
    ...
    hist->GetXaxis()->SetTitleOffset(title_offset_X);
    hist->GetYaxis()->SetTitleOffset(title_offset_Y+0.1);
    hist->GetXaxis()->SetLabelSize(label_size);
    hist->GetYaxis()->SetLabelSize(label_size);
    hist->GetXaxis()->SetTitleSize(label_size);
    hist->GetYaxis()->SetTitleSize(label_size);
    hist->GetXaxis()->SetNdivisions(505,'N');
    hist->GetYaxis()->SetNdivisions(505,'N');
    hist->GetXaxis()->CenterTitle();
    hist->GetYaxis()->CenterTitle();
    ...
    if(min_x_val != max_x_val) hist->GetXaxis()->SetRangeUser(min_x_val,max_x_val);
    if(min_y_val != max_y_val) hist->GetYaxis()->SetRangeUser(min_y_val,max_y_val);
    hist->DrawCopy(option.Data());
