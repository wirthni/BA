...
The QGP can be described as a relativistic hydrodynamic fluid with a remarkably low viscosity to entropy density ratio $\eta/s \approx 1/4\pi$ \cite{busza_heavy_2018}. Since the pressure is still high, the pressure-driven hydrodynamic expansion rapidly builds up large transverse velocities, so velocity components perpendicular to the beam axis, although they were small initially \cite{busza_heavy_2018}. A volume unit of QGP expands in all directions until the volume unit undergoes a phase transition according to the phase diagram of QCD shown in figure \ref{fig:qcdphasediadram}. This transition makes the fluid fall apart into individual hadrons. The time evolution of an exemplary heavy ion collision involving gold ions is shown in figure \ref{fig:visualizationqgpproduction}.

As previously shown ultrarelativistic collisions probe different aspects of QCD in their distinct evolutionary stages. Heavy ion collisions are the only tool we have to test QCD and its emergent phenomena under extreme conditions in the laboratory \cite{busza_heavy_2018}.
