...
The data used for measuring the diffusion wake is Pb--Pb data from ALICE RUN 3 from the years 2023-2025 at $\sqrt{s_{NN}} = 5.36\bunit{TeV}$. The raw data is being filtered for general event requirements to make the diffusion wake analysis itself CPU time efficient and disk space saving. Events are required to have
...
For the $(30,15)\bunit{GeV/c}$ trigger (figure \ref{fig:aliceresultshad3015}) the positive $\eta$ hemisphere shows an enhancement of soft hadrons in the two $p_T$ bins $0 \leq p_{T,\text{Hadron}} \leq 1 \bunit{GeV/c}$ and $1 \leq p_{T,\text{Hadron}} \leq 2 \bunit{GeV/c}$ although there is no depletion in the negative $\eta$ hemisphere. The $2 \leq p_{T,\text{Hadron}} \leq 4 \bunit{GeV/c}$ bin shows a diffusion wake signal that is solely non-zero for $\eta < 0$. The highest soft hadron $p_T$ bin shows, also across other trigger variants, a similar behavior like seen in the dijet trigger analysis.

For the $(20,15)\bunit{GeV/c}$ trigger (figure \ref{fig:aliceresultshad2015}) the lowest $p_T$ bin shows an asymmetric but significant diffusion wake signal. The $1 \leq p_T \leq 2 \bunit{GeV/c}$ bin is zero apart from an enhancement of particles for $\eta > 0$. The $2 \leq p_T \leq 4 \bunit{GeV/c}$ bin only has a small but significant depletion for $\eta < 0 $ while being zero elsewhere.

For the $(15,10)\bunit{GeV/c}$ trigger (figure \ref{fig:aliceresultshad1510}) the two lowest $p_T$ bins show the expected depletion and enhancement in opposite $\eta$ hemispheres significantly while being greater for softer hadrons. 

Since the $(15,10)\bunit{GeV/c}$ trigger has shown promising results a hadron-exclusive measurement with a $(15,8)\bunit{GeV/c}$ trigger was performed. For soft hadrons with $0 \leq p_T \leq 2\bunit{GeV/c}$ the diffusion wake signal is significant with few systematic errors. For higher $p_T$ bins there is no diffusion wake signal.

\subsection{Context of the results}

\paragraph{Comparison to other results}
...
Due to the different centrality categories the two ALICE analyses are not expected to match exactly. The comparison is shown in figure \ref{fig:aliceresultsComparison}. The results of ALICE Pb--Pb 0-10\% data and 0-30\% data have similar shape although they differ slightly quantitatively. ALICE 0-30\% data (figure \ref{fig:aliceresultsComparison}, blue) has smaller absolute values than ALICE 0-10\% data (figure \ref{fig:aliceresultsComparison}, black) on average. In the softest charged hadron bins $0 \leq p_T \leq 2\bunit{GeV/c}$ the correlation difference for 0-10\% is globally bigger, indicating a separate bias in large/small gap events. This bias either enhances the correlation for large gap events or reduces it for small gap events. The 0-30\% sample should be regarded more liable because the predicted correlation difference is asymmetric in pseudorapidity, meaning its mean value is expected to be zero. The bins holding $0 \leq p_T \leq 2 \bunit{GeV/c}$ show that this asymmetry is better in the 0-30\% sample than in the 0-10\% sample. The 0-30\% sample also has less systematic errors since for higher transverse momentum bins the diffusion wake signal must vanish which is better fulfilled than in the 0-10\% sample. The hydrodynamic prediction for 0-10\% matches the 0-30\% data better than the 0-10\% data because of the stated shift. The 0-10\% data points are slightly higher than in the prediction for $\eta < 0$ but fit well for $\eta > 0$.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Media/Plots/Dijet_Data_Comparison.pdf}
	\caption{Comparison of different result of the dijet analysis using ALICE RUN 3 data and leading/subleading jets with $(p_{T,\text{min}}^\text{Leading}, p_{T,\text{min}}^\text{Subleading}) = (40, 20)\bunit{GeV/c}$.	$C_x = 1/N_x \cdot d N_x/d \eta$ in different centrality categories. The ALICE 0-10\% data has been rebinned similar to the ALICE 0-30\% data.}
	\label{fig:aliceresultsComparison}
\end{figure}

\newpage

\paragraph{Classification of the results}
The diffusion wake analysis using a correlated dijet pair as a trigger object is biased at $|\eta| = 0.3$ consistently among all variants. Results that fail the null hypothesis test for $\eta \leq 0$ and show a depletion for $\eta \leq 0$ are shown in variants

\begin{itemize}
\item $(40,20)\bunit{GeV/c} \left\{ 1 \leq p_T \leq 2 \bunit{GeV/c} \right\} $
\item $(30,15)\bunit{GeV/c} \left\{ 1 \leq p_T \leq 2 \bunit{GeV/c} \right\} $
\item $(30,15)\bunit{GeV/c} \left\{ 2 \leq p_T \leq 4 \bunit{GeV/c} \right\} $
\item $(20,10)\bunit{GeV/c} \left\{ 1 \leq p_T \leq 2 \bunit{GeV/c} \right\} $
\item $(20,10)\bunit{GeV/c} \left\{ 2 \leq p_T \leq 4 \bunit{GeV/c} \right\} $
\end{itemize}

Among those only the $(40,20)\bunit{GeV/c} \left\{ 1 \leq p_T \leq 2 \bunit{GeV/c} \right\}$ variant could be compared to theoretical predictions quantitatively.

The diffusion wake analysis using a correlated dihadron pair as a trigger object is not affected by the anomalies at $|\eta| = 0.3$. Results that fail the null hypothesis test for $\eta \leq 0$ and show a depletion for $\eta \leq 0$ are shown in variants

\begin{itemize}
\item $(30,15)\bunit{GeV/c} \left\{ 2 \leq p_T \leq 4 \bunit{GeV/c} \right\}$
\item $(20,15)\bunit{GeV/c} \left\{ 0 \leq p_T \leq 1 \bunit{GeV/c} \right\}$
\item $(20,15)\bunit{GeV/c} \left\{ 2 \leq p_T \leq 4 \bunit{GeV/c} \right\}$
\item $(15,10)\bunit{GeV/c} \left\{ 0 \leq p_T \leq 1 \bunit{GeV/c} \right\}$
\item $(15,10)\bunit{GeV/c} \left\{ 1 \leq p_T \leq 2 \bunit{GeV/c} \right\}$
\item $(15,10)\bunit{GeV/c} \left\{ 2 \leq p_T \leq 4 \bunit{GeV/c} \right\}$
\item $(15,8)\bunit{GeV/c} \left\{ 0 \leq p_T \leq 1 \bunit{GeV/c} \right\}$
\item $(15,8)\bunit{GeV/c} \left\{ 1 \leq p_T \leq 2 \bunit{GeV/c} \right\}$
\item $(15,8)\bunit{GeV/c} \left\{ 2 \leq p_T \leq 4 \bunit{GeV/c} \right\}$
\end{itemize}

The dihadron analysis has brought better results in terms of statistical uncertainties and anomalies.
