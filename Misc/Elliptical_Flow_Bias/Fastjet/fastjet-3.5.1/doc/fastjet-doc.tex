...
\title{\sf FastJet user manual%
  \\ \large (for version
  3.5.1%  %%%-VERSION-NUMBER (do not remove this comment; used
...
Computational Geometry Algorithms Library (\ttt{CGAL})~\cite{CGAL}
...
\begin{verbatim}
 curl -O https://fastjet.fr/repo/fastjet-X.Y.Z.tar.gz 
...
%
\ttt{CGAL} can be enabled with the \verb|--enable-cgal| option at the
\ttt{configure} stage.
% \footnote{Since version 4.9, \ttt{CGAL} can
%   be used as a header-only library (this is the default from version
%   5.0 of \ttt{CGAL}),
% %In this case, \fastjet requires an installed version of CGAL.
% %as well as the \ttt{--enable-cgal-header-only}
% %configure argument, 
% see e.g.\ \href{https://doc.cgal.org/latest/Manual/usage.html\#section_headeronly}{section 4} of the \ttt{CGAL} manual.}
%
\ttt{CGAL} may be obtained in source form from
\url{http://www.cgal.org/} and is also available from package managers for
many common Linux distributions.
%
%For \ttt{CGAL} versions 3.4 and higher,
The user can specify
\verb|--with-cgaldir=...| if the \ttt{CGAL} files are not installed in a
standard location.
%
% \footnote{For events with near degeneracies in their
%   Delaunay triangulation, issues were found with versions 3.7 and
%   3.8 of \ttt{CGAL}. }

The \ttt{NlnNCam} strategy does not require \ttt{CGAL}, since it is based on
a considerably simpler computational-geometry structure~\cite{Chan}.

\subsection{With CMake}

As of version 3.5.0, FastJet can also be built with CMake, using the following 
set of commands
\begin{verbatim}
  cmake -S . -B build
  cmake --build build
  ...
  cmake --install build  
\end{verbatim}
Running \ttt{cmake build -LAH} will list the options available. Of particular 
note are 
\begin{verbatim}
  -DCMAKE_INSTALL_PREFIX=/install/path   # sets the installation prefix
  -DFASTJET_ENABLE_PYTHON=ON             # turns on build of Python interface
  -DFASTJET_ENABLE_CGAL=ON               # turns on CGAL
\end{verbatim}
Note that with CMake, the build of the Python interface requires SWIG to be installed. 


%======================================================================
\section{Python interface}

As of version 3.3.0, \fastjet ships with a Python interface.
%
Note that the \fastjet Python package that is obtained with \ttt{pip
  install fastjet} is a superset of that distributed with the \fastjet
release. Further details of that superset are to be found at
\url{https://github.com/scikit-hep/fastjet}.

%
Compilation can be enabled at configure time with the option
\ttt{--enable-pyext} (with autotools) or
\ttt{-DFASTJET\_ENABLE\_PYTHON=ON} (with CMake).
%
...
% full \fastjet package from \url{https://fastjet.fr}, are meant to provide easy access to these 
...
%
It is available for download from the \url{https://fastjet.fr/} web
site.
...
%
The \ttt{NlnN} strategy uses \ttt{CGAL}'s Delaunay Triangulation
