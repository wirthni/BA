...
The background's transverse momentum probability density will produce few high-$p_T$ particles. Due to the indifferentiability of background particles and non-background particles in real data, it is difficult to produce a transverse momentum statistics that only involves background particles. Nevertheless, because the probability density decreases exponentially low-$p_T$ particles will strongly dominate the particle spectrum, making the occasionally produced high-$p_T$ particles negligible.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Media/Plots/Simu_Event.pdf}
...
For the $(40,20)\bunit{GeV/c}$ trigger (figure \ref{fig:flowresultsjet4020}) all results are slightly biased with the tendency for an enhancement at $\eta \leq 0$ and a depletion for $\eta \geq 0$, opposing the diffusion wake signal. 

For the $(30,15)\bunit{GeV/c}$ and $(20,15)\bunit{GeV/c}$ trigger the diffusion wake opposing is bigger and the result is highly biased. All $p_T$ bins in figures \ref{fig:flowresultsjet3015} and \ref{fig:flowresultsjet2010} show this bias signal as a clear wave, inverted relative to what is proposed in figure \ref{fig:DijetProposal2}.
...
For the $(30,15)\bunit{GeV/c}$ trigger the diffusion wake signal is slightly biased except for the $4 \leq p_T \leq 6 \bunit{GeV/c}$ bin. The signal is only weakly structured and the bias is subject to big fluctuations.

For the $(20,15)\bunit{GeV/c}$ trigger the result is only unbiased in the $0 \leq p_T \leq 1 \bunit{GeV/c}$ soft hadron bin, else it is slightly biased. The signal is only weakly structured and the bias is subject to big fluctuations except for bin $4 \leq p_T \leq 6\bunit{GeV/c}$ where the signal is positive for $\eta < 0$, opposing the diffusion wake. 

For the $(15,10)\bunit{GeV/c}$ trigger the result is only unbiased in the $1 \leq p_T \leq 2 \bunit{GeV/c}$ soft hadron bin, else it is slightly biased. However, the signal is below zero for $\eta > 0$ in the $1 \leq p_T \leq 2 \bunit{GeV/c}$ bin, opposing the diffusion wake signal. The bias signal has a clear structure in the $4 \leq p_T \leq 6 \bunit{GeV/c}$ bin where it has a waveform opposing the diffusion wake signal. 

\subsubsection{Context of the results}

First the diffusion wake analysis is first conducted using a correlated dijet pair as trigger. The simulation has shown that the lower the jet trigger transverse momentum the larger a diffusion wake opposing bias becomes. The $1 \leq p_T \leq 2 \bunit{GeV/c}$ bin is part of the prediction in figure \ref{fig:diffusionwakesimu}. The diffusion wake has a deflection of $\approx -0.8 C$ for $\eta < 0$ whereas the bias has a deflection of $\approx +0.1 C$ in the same region. This means that although the $\chi^2$ test has shown that the result is clearly biased, the systematic bias is small compared to the diffusion wake signal. However, it is not negligible when investigating the diffusion wake signal quantitatively accurate. Further investigations on the observed bias have to be done to judge where it comes from. Either the signal is genuine or it is a fragment of the simulation. The simulation is highly sensitive to small parameter changes. It is possible that minor flaws of the simulation, for example the transverse momentum probability density overlaps when combining PYTHIA+BG, produce the bias signal. The event acceptance rates of the dijet triggered simulated events (table \ref{tab:SimJetAcceptance}) are in good agreement with the acceptance rates in ALICE RUN 3 data (table \ref{tab:ALICEJetAcceptanceRates}).

The diffusion wake analysis is then conducted using a correlated dihadron pair as trigger. The dihadron trigger method's results are less biased than the results of the dijet trigger method in every soft hadron $p_T$ bin. Deflections in the diffusion wake signal are smaller than in the dijet trigger method and deflections are less pronounced. The event acceptance rates for the $(20, 15)\bunit{GeV/c}$ and $(15,10)\bunit{GeV/c}$ trigger variants (table \ref{tab:SimHadAcceptance}) are low compared to the acceptance rates in ALICE RUN 3 data (table \ref{tab:ALICEHadAcceptanceRates}). The acceptance is lower by a factor of 10 for the mid trigger threshold and by a factor of 4 for the lowest trigger threshold. [TODO]
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Media/Plots/FlowResults_Had_15_10_V1Mag_100.pdf}
