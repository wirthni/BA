...
Figure \ref{fig:flowresultshad1510_V1Mag_100} shows the result of the dihadron analysis using a trigger hadron pair of $(15,10) \bunit{GeV/c}$ with a directed flow magnification of $M = 100$. The behavior is similar to the dijet trigger variant although the influence of the magnified $v_2'$ is smaller. The shape of the correlation difference for the softest particles coincides with the shape of the diffusion wake in the hydrodynamic prediction in figure \ref{fig:diffusionwakesimu}.

The comparison between the dijet and dihadron analysis in both cases, using the original flow $v_2$ and the magnified flow $v_2'$, has delivered interesting results. The signal opposing bias with the original $v_2$ is bigger for a dijet trigger than for a dihadron trigger. This is also the case when using a magnified $v_2'$. The influence is stronger when using dijets as triggers than using dihadrons. Since reconstructed jets are subject to bias through anisotropic flow (refer to section \ref{sec:analysis-of-heavy-ion-collision-data}), it is possible that these two effects are correlated.
