...
The event acceptance rates for the $(20, 15)\bunit{GeV/c}$ and $(15,10)\bunit{GeV/c}$ trigger variants (table \ref{tab:SimHadAcceptance}) are lower compared to the acceptance rates in ALICE RUN 3 data (table \ref{tab:ALICEHadAcceptanceRates}). The acceptance is lower by a factor of 10 for the mid trigger threshold and by a factor of 4 for the lowest trigger threshold. TODO: WHY ALEX, WHY????

\iffalse
TODO:
A possible explanation for this deviation lies in the provided events. The ALICE RUN 3 Pb--Pb data has been preselected for events that have at least one high-$p_T$ hadron (refer to section \ref{sec:dianaALICE}), but not the PYTHIA p-p that is used to model high-$p_T$ hadrons in the simulated events.
\fi
